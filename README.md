...
- **Recursive library scan** — loads all `.mod` / `mod.*` files from the `MODS` directory on startup
...
| `4xx`  | Vibrato (sine/ramp/square/random) |
...
Place MOD files anywhere inside the `MODS/` directory (subdirectories are scanned recursively). Files are recognised by a `.mod` extension or a `mod.` prefix.
